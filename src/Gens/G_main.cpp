...
#ifdef GENS_KMOD
	// Save before End_All releases the ROM and tears down debug windows.
	strcpy(Str_Tmp, Gens_Path);
	...
	switch(message)
	{
		...
		case WM_COMMAND:
			...
			else switch(LOWORD(wParam))
			{
                ...
                case ID_CPU_DEBUG_PLANE_A:
                    MINIMIZE
                    Change_Debug(hWnd, DMODE_PLANE_A);
                    return 0;

                case ID_CPU_DEBUG_PLANE_B:
                    MINIMIZE
                    Change_Debug(hWnd, DMODE_PLANE_B);
                    return 0;
...
HMENU Build_Main_Menu(void)
{
    ...
    MENU_L(CPUDebug, 3, Flags, ID_CPU_DEBUG_PLANE_A, "Plane A", "", "Plane A");
    MENU_L(CPUDebug, 4, Flags, ID_CPU_DEBUG_PLANE_B, "Plane B", "", "Plane B");
	...
	if (SegaCD_Started)
	{
		...
		MENU_L(CPUDebug, 6, Flags | MF_POPUP, (UINT)DebugMCD, "SegaCD", "", "SegaCD");
	...
	if (_32X_Started || CD_32X_Active)
	{
		...
		if (SegaCD_Started)
		{
			MENU_L(CPUDebug, 7, Flags | MF_POPUP, (UINT)Debug32X, "32X", "", "32X");
		}
		else
		{
